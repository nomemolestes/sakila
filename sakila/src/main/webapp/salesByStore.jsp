...
<%
	...
	//호출
	SalesByStoreDao sbsd = new SalesByStoreDao();
	SalesByStore sbs = new SalesByStore();
	List<SalesByStore> list = sbsd.selectSalesByStoreByPage(beginRow, rowPerPage);
	list = sbsd.selectSalesByStoreByPage(beginRow, rowPerPage);
			
	int lastPage = 0;//마지막페이지
	int totalCount = sbsd.selectSalesByStoreTotalRow();//전체 글의수, 전체행반환메서드 호출
